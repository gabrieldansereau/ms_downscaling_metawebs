...
First, we added an analysis of network motifs across ecoregions to further explore the variation in network structure across space beyond richness and number of links, as both reviewers suggested. We added a new figure with those results (now Fig. 4). We decided to move our previous Fig. 3 (showing the bivariate relationships between community and network measures) to Supplementary Material (now Fig. S1) to respect the Journal's limit on the number of figures. However, we will gladly put it back in the main text if the Editor judges that we have room for it.
