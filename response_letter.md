...
We would like to thank both reviewers for their helpful input on our manuscript, as well as the editor for offering us the opportunity to submit a revised version. Addressing the comments by both reviewers led to two *major* changes.

First, we added **an analysis of network motifs across ecoregions** to explore further the variation in network structure across space beyond documenting richness and number of links -- this had been suggested by both reviewers. We added a new figure with those results (now Fig. 4, attached in this response document). We decided to move our previous Fig. 3 (showing the bivariate relationships between community and network measures) to Supplementary Material (now Fig. S1) to respect the journal's limit on the number of figures. However, we will gladly put it back in the main text if the editor judges that we have room for it.

Second, we followed Reviewer 1's suggestion and **changed our pseudo-absences selection algorithm for one which weights candidate sites by their distance to an observation** (after performing spatial thinning to remove the sampling effect), making it more likely to select sites further away from observed ones. We believe that, as Reviewer 1 pointed out, doing so allows us to better represent the potential distributions of species. Consequently, we re-ran our species distribution models and updated our figures given the new distributions. These updates do not change the conclusions from our manuscript, except for one result, which we discuss in the following paragraph. Otherwise, our results are very consistent, notably regarding the latitudinal gradients of species richness and number of links, the differences in the spatial distributions of ecoregion median values and within-ecoregion variance, and the contrast between hotspots of network and species uniqueness.

The one result that differs from our previous version as a consequence of the new method to generate pseudo-absences is the mismatch in the distributions of species richness and number of links that we reported in our earlier analyses. As we can see on our updated Fig. 1 and Fig. S1, both spatial distributions now follow each other quite closely, while they showed clear differences before. Nonetheless, we believe this result follows logically from the change of focus towards potential distributions and is preferable for two reasons. First, as Reviewer 1 pointed out, modelling potential distributions and wider responses to the environment makes more sense to go along with the metaweb concept. The metaweb and its downscaled localized version we present here are highly potential in nature and aim to serve as an upper boundary for what the interactions could be at a local scale. Therefore, using potential distributions ensures we capture the widest potential response given the environment. Second, changing the pseudo-absence generation method smoothed out the results for species richness and reduced over-fitting towards the cities, which we think is more appropriate here (we attached the previous and updated richness figures for comparison below, Figs. R1-R2). As we now select pseudo-absences further away from the species occurrences, our distribution models essentially do in-filling within the observed species range and are likely to generate distributions with fewer holes. Notably, species richness increased in most areas, now reaching a maximum of around 100 compared to 85 before (as visible below). Consequently, the number of links increased even more than richness (from a maximum of about 700 links up to 850 links). Given the increase in richness, species can realize their interactions with the highest probabilities in more locations except in very poor sites. Doing so, we believe it normal to observe a better agreement between the two distributions, as there is now less of a spatial constraint on where species can realize their interactions. As we know that the number of interactions scales with the number of species, these two variables are expected to vary in space in the same way when focusing on the potential species ranges; in a sense, that we observe this result and nonetheless are able to document different variation in network structure (using motifs) shows that our approach can highlight variation in food web structure over space that is independant from the effect of species richness.

Please also find below our point-by-point response to all comments from the reviewers. Line numbers refer to the updated version of our manuscript. We additionally provide a version with tracked changes compared to our initial manuscript.
...
As mentioned above, we agree with the reviewer that we should aim to model potential distributions and wider responses to the environment and clarified it on L128-130. We followed the reviewer's suggestion and assigned a lower probability of choosing pseudo-absences near occurrences (therefore, the known species range) and a higher probability of selecting sites further away.
...
We added an analysis of network motifs to further explore network structure in space. We chose motifs over nestedness and modularity as they capture much of the variation in network structure (in particular for food webs), are linked to community persistence, and are conserved across scales, on top of having an intuitive interpretation (e.g. what type of competition between three species is more common in a network).
...
We believe that our analysis of motifs further explores the gradients in network structure in a way relevant to this manuscript. We fully agree that a deeper exploration of those gradients will be relevant in future manuscripts, in particular when coupled (as Reviewer 2 suggests) to sustained empirical validation of the predictions.
...
We would like to emphasize that the premise of our work is to generate local food web predictions when all we have is a higher level metaweb and no specific localized data, as we discuss in the last paragraphs of the Discussion. The accuracy of the predictions will of course depend on the accuracy of the data (here the metaweb). Yet, developing a way to generate actionable information when information is initially scarce is also highly important in our view. In a sense, our manuscript highlights the application of this predictive pipeline in a "worst-case scenario" situation (interactions are almost all predicted), but it can equally be applied when there are additional data about the interaction network. With the recent increase in work predicting species interactions, notably in space, we identify our work as contributing to the obvious next question of "how we do map these results in space?".
...
The European tetrapod metaweb is based on expert knowledge and literature review (L104-106), not field observations (although this also requires tremendous work). As we mentioned in the Discussion, our framework does require a metaweb, but it could have been generated through various means. It is especially useful if used with probabilistic values and uncertainty at the scale of interactions, as we do here, but it doesn't have to be. What to do when we don't have a metaweb is not the point of the current manuscript; rather it was covered by @Strydom2022FooWeb and @Strydom2023GraEmb, on which we admittedly based a lot of this work, who showed that we can transfer knowledge from other sources (as was done from Europe for the Canadian metaweb) and that building a metaweb is an essential step before getting to localized network values.
...
We added more information on the size and content of the Canadian mammal metaweb relevant to this manuscript (L100-101). We would however like to point out that we are not creating the metaweb here, but only re-using the one produced by @Strydom2022FooWeb. The metaweb is available alongside their manuscript.
...
We intend to model the potential distributions of species and wide responses to environmental conditions, which we emphasized in this revised version. Therefore, we believe it is appropriate to include occurrences outside of Canada to train our models, as it can provide information on environmentally similar sites in Canada which might not have been sampled. We recognize that our training extent is very wide and goes far beyond Canada in the hope of capturing a wide span of environmental conditions and as much of the species distributions in Canada as possible. It is a choice we made considering the goal of our study and discussed in referenced textbooks. For instance, @Guisan2017HabSui mention that using only a subset of the species range (as we would be doing if considering only observations in Canada) does not allow us to model the full niche of species and full extent of environmentally suitable conditions. Consequently, the risk of over-representating species ranges in the south seems acceptable because we intend to represent potential distributions, therefore the full niche overlapping Canada.
...
The method to select pseudo-absences does have an effect. As we explained in our general response, we believe selecting pseudo-absences further away from occurrences better represents the potential distribution of species and is more appropriate to downscale the metaweb. We added this explanation on L128-130.
...
We clarified this on L230-232. The site-by-interaction matrix is assembled with the localized probability of individual interactions at every site, based on the downscaled metaweb. Its dimensions are 970,698 sites by 3,108 interactions (the number of defined interactions in the Canadian metaweb once reconciled with GBIF taxonomy). This is analogous to the site-by-species community matrix used for species uniqueness (970,698 sites by 159 species).
...
We clarified this on L286-289. The median values represent the median across all sites in the ecoregion. First, we calculated species richness and number of links separately on all sites in the ecoregion based on the downscaled metaweb. Then, we take the median value across all sites in the ecoregion and attribute this value to the ecoregion. An ecoregion with a high median for the number of links could either have some interactions with a very high probability or many interactions with a lower probability.
...
We nuanced this statement on L357-361. We recognize this is not the best comparison (as we mention in the text). However, we think it's relevant to mention here as it is the closest comparison of a metaweb structure in space.
...
We chose these specific colors as there are very few colorblind-friendly bivariate color palettes [based on this review: <https://jakubnowosad.com/posts/2020-08-25-cbc-bp2>\; @Nowosad2020HowCho]. We kept the same colors throughout for species and network results for consistency. It is unfortunate that the colors do not stand out on grey print out, but we prefer to stick with a colorblind-friendly option.
